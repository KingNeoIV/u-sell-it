...
(
...
) 
    {
        ...
        if (user.length() < 4 || !std::isalpha(user[0]) || pass.length() <= 6) return 0;
        ...
        // First/Last name
        if (!is_alpha_str(first_name) || !is_alpha_str(last_name)) return 0;

        // Street address (basic non-empty check)
        if (std::string(street_address).empty()) return 0;

        // City/State
        if (!is_alpha_str(city) || !is_alpha_str(state)) return 0;

        // Zip code (5 digits)
        if (!is_digit_str(zip_code) || std::string(zip_code).length() != 5) return 0;

        // Phone (10 digits)
        if (!is_digit_str(phone) || std::string(phone).length() != 10) return 0;

        // Email (basic check)
        std::string mail(email);
        if (mail.find('@') == std::string::npos || mail.find('.') == std::string::npos) return 0;

        // Username must be at least 4 chars and contain letter, digit, special char
        if (user.length() < 4 || !std::isalpha(user[0])) return 0;
        ...
        try
        {
            ...
            (
                "SELECT user_id FROM users WHERE username = " + txn.quote(username) + " OR email = " + txn.quote(email)
            );
            ...
            txn.exec
            (
                "INSERT INTO users (username, email, password, first_name, last_name, phone, street_address, city, zip_code, state) "
                "VALUES (" + txn.quote(username) + ", " + txn.quote(email) + ", " + txn.quote(password) + ", "+ txn.quote(first_name) + ", " 
                + txn.quote(last_name) + ", " + txn.quote(phone) + ", " + txn.quote(street_address) + ", "+ txn.quote(city) + ", " 
